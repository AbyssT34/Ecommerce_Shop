...
async function seed() {
    try {
        ...
        for (let i = 0; i < categories.length; i++) {
            ...
            const [existing] = await dataSource.query('SELECT id FROM categories WHERE slug = ?', [cat.slug]);
            if (existing) {
                await dataSource.query('UPDATE categories SET name = ?, description = ? WHERE slug = ?', [cat.name, cat.description, cat.slug]);
                categoryIdMap[i + 1] = existing.id;
                console.log(`  ✓ Updated: ${cat.name} (ID: ${existing.id})`);
            }
            else {
                const result = await dataSource.query('INSERT INTO categories (name, slug, description) VALUES (?, ?, ?)', [cat.name, cat.slug, cat.description]);
        ...
        for (const product of productsData) {
            ...
            const [existingProduct] = await dataSource.query('SELECT id FROM products WHERE sku = ?', [product.sku]);
            if (existingProduct) {
                await dataSource.query(`UPDATE products SET 
            name = ?, 
            description = ?, 
            price = ?, 
            stock_quantity = ?, 
          ...
          WHERE sku = ?`, [
                ...
                await dataSource.query(`INSERT INTO products (name, sku, description, price, stock_quantity, image_url, category_id, is_active, unit)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                    product.name,
                    product.sku,
                    product.description,
        ...
        for (const userData of testUsers) {
            const [existingUser] = await dataSource.query('SELECT id FROM users WHERE email = ?', [userData.email]);
            const passwordHash = await bcrypt.hash(userData.password, 10);
            if (!existingUser) {
                await dataSource.query(`INSERT INTO users (email, password_hash, full_name, role, created_at, updated_at)
           VALUES (?, ?, ?, ?, NOW(), NOW())`, [userData.email, passwordHash, userData.fullName, userData.role]);
                console.log(`  ✓ Created user: ${userData.email} (password: ${userData.password})`);
            }
            else {
                await dataSource.query(`UPDATE users SET password_hash = ?, updated_at = NOW() WHERE email = ?`, [passwordHash, userData.email]);
